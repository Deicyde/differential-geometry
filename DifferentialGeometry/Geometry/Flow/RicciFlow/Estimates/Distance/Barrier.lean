...
open DifferentialGeometry.Geometry.Curvature
  (heatOperatorWithDrift_zero_drift heatOperator_eq_laplacianAt laplacianAt
    laplacianAt_eq)
open DifferentialGeometry.Geometry.Connection (LeviCivita)
open DifferentialGeometry.Geometry.Operator
  (gradientFun gradientFun_const_smul laplacian laplacian_smul_at)
...
attribute [-instance] Tensor0SBundle.tangentSpaceNormedAddCommGroup
  Tensor0SBundle.tangentSpaceNormedSpace in
private opaque completeInst
    (g : SmoothRiemannianMetric I M)
    (h : RiemannianMetricComplete (I := I) g) :
...
theorem scaled_of_quad
  ...
  let : EMetricSpace M :=
    EMetricSpace.ofRiemannianMetric I M
  let : CompleteSpace M :=
    completeInst (I := I) (g := S.base.metric t) hcomplete_t
